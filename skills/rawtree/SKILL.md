---
name: rawtree
description: Operate and query RawTree. Use for RawTree tools, CLI, or API tasks involving authentication, databases, API keys, ingestion, tables, logs, read-only SQL, nested or Dynamic fields, query debugging, and performance.
metadata:
  author: rawtree
  version: "0.4.0"
---
...
Treat tools, the CLI, and the API as access surfaces for the same RawTree workflows. Preserve the user's chosen surface when one is specified.

## Route

Read every reference whose condition matches before acting:

- For RawTree tool calls, use the tool schema as the interface contract and load the semantic references below for the requested operation.
...
An API or CLI query requires both its interface reference and `query.md`. Load the Dynamic and performance references only when their branches apply. Routing is complete when every requested operation and RawTree-specific data risk maps to a loaded reference.

## Workflow

1. Verify the input contract.
   - For RawTree tool work, inspect the tool schema, annotations, and required inputs.
